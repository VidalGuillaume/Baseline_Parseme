...
from objet_baseline import generate_texte_corrige, generate_clef, segmentation, getFile, Combinaison
...
def opt_baseline(file_texte_dev, file_texte_train, list_expression, list_argument_form=[True,False], list_argument_lemma=[True,False], list_argument_upostag=[True,False], list_argument_deprel=[True,False], list_argument_order=[True,False], list_argument_distance=[True,False], parametre_distance_verification=["0",0], display=True) :
	if display :
	...
	if file_texte_dev is None :
		...
		return "lemma_upostag_deprel_order"
	...
	while combinaison_parametre.canContinue() :
		...
		blind(texte_dev)
		project_lexicon(texte_dev, list_expression, argument_form, argument_lemma, argument_upostag, argument_deprel, argument_order, argument_distance, parametre_distance_verification[1], display=False)
		
		with open("baseline_result/dev_result/dev_" + clef + ".txt", 'w') as mon_fichier :
		...
		file = getFile()
		os.system(file + "bin/evaluate.py --pred baseline_result/dev_result/dev_" + clef + ".txt --gold " + file_texte_dev + " --train " + file_texte_train + " >> baseline_result/dev_result/scoredev_" + clef + ".txt")
...
if __name__ == '__main__' :
	...
	list_expression, [parametre_min, parametre_max, parametre_moyenne, parametre_variance] = extract_lexicon(parse(generate_texte_corrige(parser.parse_args().train)), display=True)
	with open("baseline_result/temp_result/temp_lexicon.txt", 'w') as mon_fichier :
