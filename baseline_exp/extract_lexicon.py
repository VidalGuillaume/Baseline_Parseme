...
from objet_baseline import generate_texte_corrige, ExpressionComplete, ExpressionWord, GroupExpression
import os
import time
...
def traitement_donne(list_donne) :
	if len(list_donne) == 0 :
		parametre_min = 0
		parametre_max = 0
		parametre_moyenne = 0
	...
	return [parametre_min, parametre_max, parametre_moyenne, parametre_variance]
...
def extract_lexicon(texte_train, display=True) :
	...
	for paragraphe_TokenList in texte_train :
		...
		for expression in list_temp :
			...
			if not expression[0] in dict_expression :
				dict_expression[expression[0]] = []
				dict_donne[expression[0]] = []
			boolean = True
			for i in dict_expression[expression[0]] :
				if i.semi_equals(expression_complete) :
					boolean = False
					i.fusion(expression_complete)
			if boolean :
				dict_expression[expression[0]].append(expression_complete)
			...
			for mot in paragraphe_TokenList :
				...
				if (mot['id'] > temp_taille_min) and (mot['id'] < temp_taille_max) :
					if not mot in expression :
						temp_compteur += 1
			dict_donne[expression[0]].append(temp_compteur)

	if display :
		print(str(round(time.time() - time_debut)) + '\n', file=os.sys.stderr)

	list_expression_type = []
	for key in dict_expression.keys() :
		list_expression_type.append(GroupExpression(key,dict_expression[key],traitement_donne(dict_donne[key])))
	return list_expression_type # pour le opt_lexicon
...
if __name__ == "__main__" :
	...
	list_expression_type = extract_lexicon(parse(generate_texte_corrige(parser.parse_args().train)), display=False)
