...
from objet_baseline import generate_texte_corrige, segmentation, decode_key, Combinaison, ExpressionComplete, ExpressionWord, GroupExpression, ArgumentSelectionExpression
...
def generate_expression_id(type_expression, expression, temp_combinaison) :
	resultat = type_expression
...
def project_lexicon(texte_test, list_expression_type_argument, display=True) :
	if display :
	...
	for i in list_expression_type_argument :
		argument_form, argument_lemma, argument_upostag, argument_deprel, argument_order, argument_distance, valeur_parametre_distance_verification = i[1].getForm(), i[1].getLemma(), i[1].getUpostag(), i[1].getDeprel(), i[1].getOrder(), i[1].getDistance(), i[1].getDistanceVerification()[1]

		if not (argument_form or argument_lemma or argument_upostag or argument_deprel or argument_order or argument_distance) :
			print("Error: lecixon " + i[0].getTypeExpression() + " contains no parameters", file=os.sys.stderr) # si il n y a pas de parametre, alors on ne peut pas rechercher d expression
			os.sys.exit()
		if not (argument_form or argument_lemma) :
			print("Error: lecixon " + i[0].getTypeExpression() + " need parameter form or lemma", file=os.sys.stderr) # il faut au moisn le parametre form ou lemma si l on veut faire une recherche correct
			os.sys.exit()

	for paragraphe_TokenList in texte_test :
		sequence_annote = []
		compteur = 1		
		for expression_argument in list_expression_type_argument :
			expression_type = expression_argument[0].getTypeExpression()
			argument_form, argument_lemma, argument_upostag, argument_deprel, argument_order, argument_distance, valeur_parametre_distance_verification = expression_argument[1].getForm(), expression_argument[1].getLemma(), expression_argument[1].getUpostag(), expression_argument[1].getDeprel(), expression_argument[1].getOrder(), expression_argument[1].getDistance(), expression_argument[1].getDistanceVerification()[1]
			for expression in expression_argument[0].getListExpression() :
				...
				while combinaison.canContinue() :
					...
					if methode_verification_supplementaire(expression, temp_combinaison, paragraphe_TokenList, argument_deprel, argument_order, argument_distance, valeur_parametre_distance_verification) :
						expression_id = generate_expression_id(expression_type, expression, temp_combinaison)
						...
						for mot_combinaison in temp_combinaison :
							if mot_combinaison.get('PARSEME:MWE') == '_' :
								mot_combinaison['PARSEME:MWE'] = str(compteur) + ((':' + expression_type) if mot_combinaison.get('id') == min_id else '')
							else :
								mot_combinaison['PARSEME:MWE'] += ';' + str(compteur) + ((':' + expression_type) if mot_combinaison.get('id') == min_id else '')
						compteur += 1
...
if __name__ == "__main__" :
	...
	for paragraphe_type_expression in temp :
		if paragraphe_type_expression == "" :
			continue

		paragraphe_type_expression = paragraphe_type_expression.split('\n\n')
		partie_parametre = paragraphe_type_expression[0].split('\n')
		partie_expression = paragraphe_type_expression[1:]
		...
		type_expression = partie_parametre[1]
		temp_distance = partie_parametre[2].split(';')
		...
		group_expression = GroupExpression(type_expression, liste_expression, [parametre_distance_min, parametre_distance_max, parametre_distance_ave, parametre_distance_var])
		argument_selection_expression = decode_key(key, parametre_distance_min, parametre_distance_max, parametre_distance_ave, parametre_distance_var)

		list_expression_type_argument.append([group_expression, argument_selection_expression])
	...
	project_lexicon(texte_test, list_expression_type_argument, display=False)
